...
bool NxpNci_ReaderActivateNext(NxpNci_RfIntf_t *pRfIntf)
{
	...
	if((Answer[0] != 0x41) && (Answer[1] != 0x06) && (Answer[3] != 0x00)) return false;
	NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_100MS);
	if((Answer[0] != 0x61) && (Answer[1] != 0x06)) return false;

	NCIRfDiscoverSelect[4] = gNextTag_Protocol;
	NCIRfDiscoverSelect[5] = (gNextTag_Protocol == PROT_ISODEP ? INTF_ISODEP : (gNextTag_Protocol == PROT_NFCDEP ? INTF_NFCDEP : INTF_FRAME));
...
bool NxpNci_WaitForDiscoveryNotification(NxpNci_RfIntf_t *pRfIntf)
{
	...
	else /* RF_DISCOVER_NTF */
	{
		...
		NCIRfDiscoverSelect[5] = (pRfIntf->Protocol == PROT_ISODEP ? INTF_ISODEP : (pRfIntf->Protocol == PROT_NFCDEP ? INTF_NFCDEP : INTF_FRAME));
