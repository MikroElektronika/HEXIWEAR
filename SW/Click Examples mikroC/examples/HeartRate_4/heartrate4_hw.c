...
void hr4_set_registers() 
{
    ...
    hr4_set_spo2_sample_rate( 0x05 );      // 32 sample rate
...
void hr4_set_led_pulse_amplitude (uint32_t amplitude)
{
    ...
    tempbuffer[0] = 0x0C;
    tempbuffer[1] = (uint8_t)(amplitude);
    tempbuffer[2] = (uint8_t)(amplitude >> 8);
    tempbuffer[3] = (uint8_t)(amplitude >> 16);
    hal_hr4_write(HR4_I2C_ADDRESS, tempbuffer, 4);
    hr4_write_reg(0x10, tempbuffer[4]);
 }
