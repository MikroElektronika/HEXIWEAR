...
(
        ...
        T_EERAM_I2C_SetSlaveAddress fpSlaveAddr,
        T_EERAM_I2C_Write fpWrite, T_EERAM_I2C_Read fpRead
    #endif
...
void EERAM_command (uint8_t command)
{
     ...
     writeData [0] = 0x55;
     writeData [1] = command;
     i2cAddress = _EERAM_ADDRESS_CONTROL;
...
{
    char writeData [2];
    writeData [0] = addressH;
    writeData [1] = addressL;
    i2cAddress = _EERAM_ADDRESS_SRAM;
    halTransfer (writeData, 2, pDataOut, countOut);
                 ...
                 uint8_t * pDataIn, uint8_t countIn)
{
    ...
    for (i = 0; i < countIn; i++)
        writeData [2+i] = pDataIn [i];
        
    halWrite (writeData, 2+countIn);
}
