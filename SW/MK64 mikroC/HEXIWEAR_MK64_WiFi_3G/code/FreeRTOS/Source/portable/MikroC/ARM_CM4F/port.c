...
static void prvPortStartFirstTask()
{
    asm {
        ...
        MSR CPU_MSP, R0
        // Clear the bit that indicates the FPU is in use in case the FPU was
        // used before the scheduler was started - which would otherwise result
        // in the unnecessary leaving of space in the SVC stack for lazy saving
        // of FPU registers.
        MOV R0, #0
        MSR CPU_CONTROL, R0
...
BaseType_t xPortStartScheduler()
{
    ...
    #if (configASSERT_DEFINED == 1)
    {
        ...
        *pucFirstUserPriorityRegister = portMAX_8_BIT_VALUE;
        ...
        while (
            (ucMaxPriorityValue & portTOP_BIT_OF_BYTE) == portTOP_BIT_OF_BYTE)
        {
            ulMaxPRIGROUPValue--;
    ...
    HEXIWEAR_SetIntPriority(
        IVT_INT_PendableSrvReq, configKERNEL_INTERRUPT_PRIORITY);
    HEXIWEAR_SetIntPriority(
        IVT_INT_SysTick, configKERNEL_INTERRUPT_PRIORITY);
...
const uint8_t ucMaxSyscallInterruptPriority
    = configMAX_SYSCALL_INTERRUPT_PRIORITY;
void xPortPendSVHandler()
#ifdef MK_MCU
iv IVT_INT_PendableSrvReq
#else
iv IVT_INT_PendSV
#endif
ics ICS_OFF
...
void xPortSysTickHandler() iv IVT_INT_SysTick ics ICS_AUTO
{
...
//   uint8_t ucCurrentPriority;
    ...
    if( ulCurrentInterrupt >= portFIRST_USER_INTERRUPT_NUMBER )
    {
        ...
        ucCurrentPriority = *((uint8_t *)(&NVIC_IPR0)
            + ulCurrentInterrupt - portFIRST_USER_INTERRUPT_NUMBER);
            
        //ucCurrentPriorityAddr = (uint8_t *)(&NVIC_IPR0)
        //    + ulCurrentInterrupt - portFIRST_USER_INTERRUPT_NUMBER;
        //ucCurrentPriority = *ucCurrentPriorityAddr;
    ...
    configASSERT((SCB_AIRCR & portPRIORITY_GROUP_MASK)
        <= ulMaxPRIGROUPValue);
}
