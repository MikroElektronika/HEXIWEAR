...
static linkStateWiFi_t watch_wifi_linkState = linkState_wifi_disconnected;
static linkState3G_t watch_3g_linkState = linkState_3g_disconnected;

static event_t watch_event;

static bool connectivity_active_wifi = false;
static bool connectivity_active_3g = false;
...
//        OSA_TimeDelay( 500 );
        ...
        switch( watch_dataPacket.type )
        {
            ...
            {
                if (connectivity_active_wifi == false && connectivity_active_3g == false)
                {
...
static void watch_GuiUpdateTask( task_param_t param )
{
    ...
    while( 1 )
    {
        ...
        {
            GuiDriver_ImageDraw( &watch_imgTempUnit );
        ...
        if( setFlags & linkStateUpdate_eventFlag )
        {
            ...
            else if (watch_linkState == linkState_disconnected)
            {
                watch_imgBluetoothLogo.img = watch_bluetooth_logo_white_bmp;
            }
            
            if( watch_wifi_linkState == linkState_wifi_connected )
            {
                watch_imgWiFiLogo.img = wifi_logo_blue_bmp;
            } else if (watch_wifi_linkState == linkState_wifi_disconnected)
            {
                watch_imgWiFiLogo.img = wifi_logo_white_bmp;
            }
            
            if( watch_3g_linkState == linkState_3g_connected )
            {
                watch_img3gLogo.img = _3g_logo_blue_bmp;
            } else if (watch_3g_linkState == linkState_3g_disconnected)
            {
                watch_img3gLogo.img = _3g_logo_white_bmp;
            }
            

            GuiDriver_ImageDraw( &watch_imgBluetoothLogo );
            if (connectivity_active_wifi==true)
            {
                GuiDriver_ImageDraw( &watch_blackImg);
                GuiDriver_ImageDraw( &watch_imgWiFiLogo );
            } else if (connectivity_active_3g==true)
            {
                GuiDriver_ImageDraw( &watch_blackImg);
                GuiDriver_ImageDraw( &watch_img3gLogo );
            }
        }
...
{
    OSA_EventSet( &watch_event, ( event_flags_t )linkStateUpdate_eventFlag );
    watch_wifi_linkState = new_linkState;
    
    if (watch_wifi_linkState == linkState_wifi_off)
    {
       connectivity_active_wifi = false;
    } else if (watch_wifi_linkState == linkState_wifi_on)
    {
       connectivity_active_wifi = true;
    }
...
{
    OSA_EventSet( &watch_event, ( event_flags_t )linkStateUpdate_eventFlag );
    watch_3g_linkState = new_linkState;

    if (watch_3g_linkState == linkState_3g_off)
    {
       connectivity_active_3g = false;
    } else if (watch_3g_linkState == linkState_3g_on)
    {
       connectivity_active_3g = true;
    }
...
void watch_Init( void *param )
{
    ...
    if( ( GuiDriver_LabelCreate( &watch_labelTemp ) == GUI_STATUS_SUCCESS )
        &&  ( currTemp != 0xFF ) && (connectivity_active_wifi == false)
        && (connectivity_active_3g == false))
    {
        GuiDriver_ImageAddToScr( &watch_imgTempUnit );
        sprintf( ( char* )watch_labelTemp.caption, "%d", currTemp); // 4,
    ...
    if (connectivity_active_wifi==true)
    {
         GuiDriver_ImageAddToScr( &watch_blackImg);
         GuiDriver_ImageAddToScr( &watch_imgWiFiLogo );
    } else if (connectivity_active_3g==true)
    {
         GuiDriver_ImageAddToScr( &watch_blackImg);
         GuiDriver_ImageAddToScr( &watch_img3gLogo );
    }
