// File: dspi_edma_master_driver.c

// Definitions of master functionalities in the DSPI module.
...
// Intermediate 16-bit command and 16-bit data buffer
static uint32_t s_cmdData;
// Consists of the last command and the final source data
static uint32_t s_lastCmdData;
// Word to send, if no send buffer, this variable is used as the word to send,
// which should be initialized to 0.
// Needs to be static and stored in data section
// as this variable address is the source address if no source buffer.
static uint16_t s_wordToSend;
// If no receive buffer provided, direct rx DMA channel to this destination
static uint8_t s_rxBuffIfNull;

// SPI instance table.
static const uint32_t spiGateTable[] =
{
    172,
    173,
    76
...
// Initiates (starts) a transfer using DMA.
...
{
    // instantiate local variable of type dspi_edma_master_state_t
    // and point to global state
    dspi_edma_master_state_t *dspiEdmaState =
        ( dspi_edma_master_state_t* )g_dspiStatePtr[instance];
    ...
    uint32_t *base = g_dspiBase[instance];
    ...
    // has preconfigured the bus using DSPI_DRV_EdmaMasterConfigureBus().
    // Do nothing for calculatedBaudRate. If the user wants to know
    // the calculatedBaudRate then they can call this function separately.
    if( device )
    {
        DSPI_DRV_EdmaMasterConfigureBus(
            instance, device, &calculatedBaudRate );
        // update bits/frame
        dspiEdmaState->bitsPerFrame = device->dataBusConfig.bitsPerFrame;
    }

    // Check the transfer byte count. If bits/frame > 8, meaning 2 bytes,
    // and if the transfer byte count is an odd count we'll have to increase
    // the TX transfer byte count by one and assert a flag to indicate
    // to the send functions that it will need to handle an extra byte.
    // For receive, we actually round down the transfer count
    // to the next lowest even number.
    ...
    if( ( dspiEdmaState->bitsPerFrame > 8 ) && ( transferByteCount & 1UL ) )
    {
        ...
        // Increment to next even byte count
        txTransferByteCnt = transferByteCount + 1U;
        // Decrement to next even byte count
        rxTransferByteCnt = transferByteCount & ~1U;
    }
    else
    {
    ...
    // Store the receiveBuffer pointer and receive byte count
    // to the run-time state struct for later use in the interrupt handler.
    ...
    // Reset the transfer counter to 0. Normally this is done via the
    // PUSHR[CTCNT], but as we are under DMA controller, we won't be able to
    // change this bit dynamically after the first word is transferred.
    ...
    // Enable the End Of Queue interrupt, which will set when DSPI sees EOQ bit
    // set in the last data word being sent. The ISR should clear this flag.
    DSPI_HAL_SetIntMode( base, kDspiEndOfQueue, true );

    // Each DMA channel's CSR[DONE] bit may be set if a previous transfer
    // occurred. The DONE bit, as the name implies, sets when the channel is
    // finished (completed it's MAJOR LOOP). The DONE needs to be cleared before
    // programming the channel's TCDs for the next transfer.
    dmaBaseAddr = VIRTUAL_CHN_TO_EDMA_MODULE_REGBASE(
        dspiEdmaState->dmaCmdData2Fifo.channel );
    dmaChannel = VIRTUAL_CHN_TO_EDMA_CHN(
        dspiEdmaState->dmaCmdData2Fifo.channel );
    EDMA_HAL_ClearDoneStatusFlag(
        dmaBaseAddr, ( edma_channel_indicator_t )dmaChannel );
    EDMA_HAL_HTCDClearReg( dmaBaseAddr, dmaChannel );

    dmaBaseAddr = VIRTUAL_CHN_TO_EDMA_MODULE_REGBASE(
        dspiEdmaState->dmaSrc2CmdData.channel );
    dmaChannel = VIRTUAL_CHN_TO_EDMA_CHN(
        dspiEdmaState->dmaSrc2CmdData.channel );
    EDMA_HAL_ClearDoneStatusFlag(
        dmaBaseAddr, ( edma_channel_indicator_t )dmaChannel );
    ...
    dmaBaseAddr = VIRTUAL_CHN_TO_EDMA_MODULE_REGBASE(
        dspiEdmaState->dmaFifo2Receive.channel );
    dmaChannel = VIRTUAL_CHN_TO_EDMA_CHN(
        dspiEdmaState->dmaFifo2Receive.channel );
    EDMA_HAL_ClearDoneStatusFlag(
        dmaBaseAddr, ( edma_channel_indicator_t )dmaChannel );
    ...
    // This means we only have one byte to read of a 16-bit data word
    // and will read this in the complete transfer function.
    
    // If a receive buffer is used and if rxTransferByteCnt > 0
    if( rxTransferByteCnt )
    {
        // For each transfer control descriptor set up, save off DMA instance
        // and channel number to simplified variable names to make code cleaner
        dmaBaseAddr = VIRTUAL_CHN_TO_EDMA_MODULE_REGBASE(
            dspiEdmaState->dmaFifo2Receive.channel );
        dmaChannel = VIRTUAL_CHN_TO_EDMA_CHN(
            dspiEdmaState->dmaFifo2Receive.channel );
        ...
        EDMA_HAL_HTCDSetSrcAddr(
            dmaBaseAddr, dmaChannel, DSPI_HAL_GetPoprRegAddr( base ) );

        // Source addr offset is 0 as source addr never increments
        EDMA_HAL_HTCDSetSrcOffset( dmaBaseAddr, dmaChannel, 0 );

        // The source and destination attributes (bit size)
        // depends on bits/frame setting
        if( dspiEdmaState->bitsPerFrame <= 8 )
        {
            ...
            // Set MAJOR count to remaining receive byte count.
            // Configure both the CITER and BITER fields.
            EDMA_HAL_HTCDSetMajorCount(
                dmaBaseAddr, dmaChannel, ( uint32_t )rxTransferByteCnt );
        }
        // For 16-bit words, but the receive buffer is still an 8-bit buffer
        else
        {
            ...
            // Set MAJOR count to remaining receive byte count.
            // Configure both the CITER and BITER fields.
            // Divide by 2 to account for minor loop of 2 bytes
            EDMA_HAL_HTCDSetMajorCount(
                dmaBaseAddr, dmaChannel, ( uint32_t )rxTransferByteCnt / 2 );
        }

        // Don't increment source address, it is constant
        EDMA_HAL_HTCDSetSrcLastAdjust( dmaBaseAddr, dmaChannel, 0 );

        // If no receive buffer then disable incrementing the destination
        // and set the destination to a temporary location.
        // Always handle rx operations to avoid rx overrun.
        if( !receiveBuffer )
        {
            // Destination is the "throw away" receive buffer
            EDMA_HAL_HTCDSetDestAddr(
                dmaBaseAddr, dmaChannel, ( uint32_t )( &s_rxBuffIfNull ) );
        ...
        else // Receive buffer is used
        {
            ...
            EDMA_HAL_HTCDSetDestAddr(
                dmaBaseAddr, dmaChannel, ( uint32_t )( receiveBuffer ) );
            // Dest addr offset, increment to the next byte
            EDMA_HAL_HTCDSetDestOffset( dmaBaseAddr, dmaChannel, 1 );
        }

        // For DSPI instances with shared RX/TX DMA requests,
        // we'll use the RX DMA request to trigger ongoing transfers
        // and will link to the TX DMA channel from the RX DMA channel.
        if( FSL_FEATURE_DSPI_HAS_SEPARATE_DMA_RX_TX_REQn( instance ) )
        {
            // Disable channel linking
            // since we have separate RX and TX DMA requests
            EDMA_HAL_HTCDSetChannelMinorLink(
                dmaBaseAddr, dmaChannel, 0, false );
        }
        else // For shared RX/TX DMA Requests
        {
            // Enable channel linking to TX channel
            // (at the end of each minor loop)
            EDMA_HAL_HTCDSetChannelMinorLink( dmaBaseAddr, dmaChannel,
                VIRTUAL_CHN_TO_EDMA_CHN(
                dspiEdmaState->dmaCmdData2Fifo.channel ), true );
            // Enable MAJOR link and link to TX DMA channel. This is needed
            // to perform one more channel link when the major loop is exhausted.
        ...
        // so that we don't keep servicing requests
        EDMA_HAL_HTCDSetDisableDmaRequestAfterTCDDoneCmd(
            dmaBaseAddr, dmaChannel, true );

        // Now that TCD was set up,
        // enable the DSPI Peripheral Hardware request for the RX FIFO
        EDMA_HAL_SetDmaRequestCmd(
            dmaBaseAddr,( edma_channel_indicator_t )dmaChannel, true );

        // Enable the Receive FIFO Drain Request as a DMA request
        DSPI_HAL_SetRxFifoDrainDmaIntMode( base, kDspiGenerateDmaReq, true );
    }

    // Set up the Last Command/data Word Intermediate Buffer
    // and fill up the TX FIFO.
    
    // Before sending the data,
    // we first need to initialize the data command struct
    // Configure the data command attributes for the desired PCS, CTAR,
    // and continuous PCS which are derived from the run-time state struct
    ...
    // If a send buffer was provided, the word comes from there.
    // Otherwise we just send a zero (initialized above).
    if( sendBuffer )
    {
        ...
        // Store the last two bytes in the send buffer
        else
        {
            // If 16-bit transfers and odd transfer byte count
            // then an extra byte was added to the transfer byte count,
            // but we cannot access more of the send buffer
            if( !dspiEdmaState->extraByte )
            {
                // Save off the last byte
                lastWord = sendBuffer[ txTransferByteCnt - 1 ];
                // Shift to MSB (separate step due to MISHA)
                lastWord = lastWord << 8U;
            }
            // OR with next to last byte
            lastWord |= sendBuffer[ txTransferByteCnt - 2 ];
        }
    }

    // Now, build the last command/data word intermediate buffer
    
    // Always clear CONT for last data word
    ...
    // 2. Set up the DMA Software Transfer Control Descriptors (STCD)
    ...
    // restore the isChipSelectContinuous setting
    // to the original value as it was cleared above
    command.isChipSelectContinuous = dspiEdmaState->isChipSelectContinuous;
    // Clear End of Queue (previously set for last cmd/data word)
    command.isEndOfQueue = 0;
    s_cmdData = DSPI_HAL_GetFormattedCommand( base, &command );

    // Place the next data from the send buffer into the intermediate buffer
    // (preload it) based on whether it is one byte or two.
    if( dspiEdmaState->bitsPerFrame <= 8 )
    {
        // If a send buffer was provided, the word comes from there.
        // Otherwise we just send a zero (initialized above).
    ...
    else
    {
        ...
        if( sendBuffer )
        {
            ...
            //  Note, if the extraByte flag is set and we're down to the last
            // two bytes we can still do this even though we're going past the
            // sendBuffer size. We're just reading data we don't care about
            // anyways since it is dummy data to make up for the last byte.
    ...
    // This channels links to "Channel 1" on completion of MAJOR loop.  
    // "Channel 1" is the channel that transfers data
    // from the send source buffer to the intermediate command/data buffer.
    // Note that the channel linking is based on the MAJOR loop complete
    // and not on minor loop.  This is because we are only sending
    // one 32-bit word so when the minor loop completes,
    // the MAJOR loop also completes.  The eDMA does not channel link on the
    // last iteration of the MAJOR loop using the ELINK mechanism, hence we have
    // to link on the MAJOR loop completion using MAJORLINK channel linking.
    ...
    if( txTransferByteCnt == 0 )
    {
        ...
        EDMA_HAL_HTCDSetSrcAddr(
            dmaBaseAddr, dmaChannel, ( uint32_t )( &s_lastCmdData ) );
    ...
    else
    {
        ...
        EDMA_HAL_HTCDSetSrcAddr(
            dmaBaseAddr, dmaChannel, ( uint32_t )( &s_cmdData ) );

        // Set the MAJOR link channel to link to the next channel
        // that will pull data from the source buffer
        // into the intermediate command/data buffer and enable MAJOR link
        EDMA_HAL_HTCDSetChannelMajorLink( dmaBaseAddr, dmaChannel,
            VIRTUAL_CHN_TO_EDMA_CHN(
            dspiEdmaState->dmaSrc2CmdData.channel ), true );

        // Do not disable ERQ request at end of major count
        EDMA_HAL_HTCDSetDisableDmaRequestAfterTCDDoneCmd(
            dmaBaseAddr, dmaChannel, false );
    }
    ...
    EDMA_HAL_HTCDSetDestAddr(
        dmaBaseAddr, dmaChannel, DSPI_HAL_GetMasterPushrRegAddr( base ) );

    // No dest addr offset, since we never increment the dest addr
    EDMA_HAL_HTCDSetDestOffset( dmaBaseAddr, dmaChannel, 0 );

    // We are only sending one 32-bit word, so MAJOR count is "1".
    // Do not use "ELINK" to link channels, use MAJORLINK in CSR,
    // therefore disable minor link (ELINK=0)
    ...
    // Implement the following DMA channel set up
    // only if we still have data yet to send.
    // Otherwise, bypass this and enable the DSPI Transmit DMA request.
    if( txTransferByteCnt != 0 )
    {
        // Scatter/gather STCD set up
        // STCD ONLY for Last intermediate command/data to PUSHR.
        // Do not call edma_hal_stcd_push_to_htcd
        // as this is used for "Channel 2" scatter/gather.
        // Hence we will call this "Channel 2 prime". This needs to be defined
        // before setting up "Channel 2" as it needs the address for this STCD.
        // IMPORTANT: Per eDMA spec, the pointer address for this STCD structure
        // needs to be aligned on 32-byte boundaries.
        
        // This channel should use scatter gather method,
        // first configure the last STCD
        memset( &config, 0, sizeof( edma_transfer_config_t ) );
        memset( dspiEdmaState->stcdSrc2CmdDataLast, 0,
            sizeof( edma_software_tcd_t ) );

        // Fill out members of the EDMA transfer config structure
        // and then use this structure to prepare
        // the software transfer control descriptor stcdSrc2CmdDataLast
        
        // Source addr is last data + last command
        config.srcAddr = ( uint32_t )( &s_lastCmdData );
        config.srcOffset = 0;
        config.srcTransferSize = kEDMATransferSize_4Bytes;
        config.destTransferSize = kEDMATransferSize_4Bytes;
        // Destination is the command/data buffer
        config.destAddr = ( uint32_t )( &s_cmdData );
        config.destOffset = 0;
        ...
        // If at this point we are left with only sending one more
        // data byte/word, then this is the last command/data to send. 
        // So the transfer control descriptor should move the
        // last command/data word into the intermediate buffer and this
        // will get transferred to the DSPI when the FIFO is ready for this.
        if( ( ( dspiEdmaState->bitsPerFrame <= 8 ) &&
                ( ( txTransferByteCnt - 1 ) == 0 ) ) ||
            ( ( dspiEdmaState->bitsPerFrame > 8 ) &&
                ( ( txTransferByteCnt - 2 ) == 0 ) ) )
        {
        ...
        // Otherwise, we are left with more data to send,
        // so use the transfer control descriptor that will move data
        // from the send source buffer to the intermediate command/data buffer.
        else
        {
            // Transfer Control Descriptor set up for Source buffer to
            // intermediate command/data (this is a linked channel).
            // AKA "Channel 2". Note, actual channel number may vary
            // depending on DMA system usage
            // This channel is triggered by the completion of "Channel 1".
            // It transfers data from the send source buffer
            // to the intermediate command/data word. When the source buffer
            // transfers the word before the last data word,
            // the MAJOR loop completes and triggers the scatter/gather
            // (ESG = 1) and loads the STCD that is set up to
            ...
            // If a send buffer was provided, the word comes from there.
            // Otherwise we set the source address to point
            // to the s_wordToSend variable that was set to 0.
            if( sendBuffer )
            {
                // Source addr is the "send" data buffer
                EDMA_HAL_HTCDSetSrcAddr(
                    dmaBaseAddr, dmaChannel, ( uint32_t )( sendBuffer ) );
            ...
            if( dspiEdmaState->bitsPerFrame <= 8 )
            {
                ...
                // Major loop count is equal to remaining number of bytes
                // to send minus 1. That is because the last data byte/word
                // is written to the last command/data intermediate buffer.
                EDMA_HAL_HTCDSetMajorCount(
                    dmaBaseAddr, dmaChannel, ( txTransferByteCnt - 1 ) );
            }
            else
            {
                ...
                // Major loop count is equal to remaining number
                // of 16-bit words to send hence we need to convert
                // remainingSendByteCount from byte to 16-bit word
                EDMA_HAL_HTCDSetMajorCount(
                    dmaBaseAddr, dmaChannel, ( txTransferByteCnt - 2 ) / 2 );
            }
            ...
            // Place the address of the scatter/gather in order to reload STCD
            // for the last command/data word to be loaded
    ...
    // For DSPI instances with separate RX/TX DMA requests,
    // we'll use the TX DMA request to trigger the TX DMA channel
    // hence we'll enable the TX channel DMA request.
    if( FSL_FEATURE_DSPI_HAS_SEPARATE_DMA_RX_TX_REQn( instance ) )
    {
        // Now that the TCD was set up for each channel, enable the DSPI
        // peripheral hardware request for the first TX DMA channel.
        EDMA_HAL_SetDmaRequestCmd(
            dmaBaseAddr, ( edma_channel_indicator_t )dmaChannel, true );
    ...
    // For DSPI instances with shared RX/TX DMA requests, we'll use
    // the RX DMA request to trigger ongoing transfers that will link
    // to the TX DMA channel from the RX DMA channel. So, we'll disable
    // the TX channel DMA request and then we'll have to manually start
    // the TX DMA channel to get the tranfer process started, where the RX
    // DMA channel will take care of the ongoing transfers from there.
    else // For shared RX/TX DMA requests
    {
        // Disable the DSPI TX peripheral hardware request
        EDMA_HAL_SetDmaRequestCmd(
            dmaBaseAddr, ( edma_channel_indicator_t )dmaChannel, false );
...
static void DSPI_DRV_EdmaMasterCompleteTransfer( uint32_t instance )
{
    ...
    // If an odd transfer count was provided when bits/frame > 8,
    // then there will be an extra byte received. Get this byte now
    // and put it into the receive buffer if a receive buffer was provided.
    if( ( dspiEdmaState->extraByte ) && ( dspiEdmaState->rxBuffer ) )
    {
        // copy the final byte from the DSPI data register to the receive buffer
        dspiEdmaState->rxBuffer[ dspiEdmaState->rxTransferByteCnt ] =
...
// Enables clock for a SPI module.
static void CLOCK_SYS_EnableSpiClock( uint32_t instance )
{
    ...
    gate = spiGateTable[ instance ];
    
    reg = gate >> 5;
    off = gate & 0x1F;
    
    *( ( uint32_t* )&SIM_SCGC1 + reg ) |= 1 << off;
}

// Disables clock for a SPI module.
...
// Returns the frequency of the SPI module.
    ...
    edma_software_tcd_t *stcdSrc2CmdDataLast )
{
    ...
    if( ( dspiEdmaState == NULL ) || ( userConfig == NULL ) ||
        ( stcdSrc2CmdDataLast == NULL ) )
    {
    ...
    // Note, remember to first enable clocks to the DSPI module
    // before making any register accesses. Enable clock for DSPI.
    ...
    // set the dspiEdmaState struct CTAR
    dspiEdmaState->whichCtar = userConfig->whichCtar;
    // set the dspiEdmaState struct whichPcs
    dspiEdmaState->whichPcs = userConfig->whichPcs;
    // continuous PCS
    dspiEdmaState->isChipSelectContinuous = userConfig->isChipSelectContinuous;

    // Initialize the DSPI module registers to default value,
    // which disables the module.
    DSPI_HAL_Init( base );
    ...
    DSPI_HAL_SetPcsPolarityMode(
        base, userConfig->whichPcs, userConfig->pcsPolarity );
    ...
    // Save runtime structure pointers to irq handler
    // can point to the correct state structure
    g_dspiStatePtr[ instance ] = dspiEdmaState;

    // enable the interrupt
    NVIC_IntEnable( g_dspiIrqId[ instance ] );

    // DSPI system enable
    DSPI_HAL_Enable( base );

    // Request DMA channels from the EDMA peripheral driver.
    // Note, some MCUs have a separate RX and TX DMA request
    // for each DSPI instance, while other MCUs have a separate RX and TX
    // DMA request for DSPI instance 0 only and shared DMA requests
    // for all other instances. Therefore, use the DSPI feature file
    // to distinguish how to request DMA channels between the various
    // MCU DSPI instances. For DSPI instances with shared RX/TX DMA requests,
    // we'll use the RX DMA request to trigger ongoing transfers
    // and will link to the TX DMA channel from the RX DMA channel.
    if( instance == 0 )
    {
        ...
        if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI0Rx,
        ...
        if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI0Tx,
    ...
    {
// Continue only if the MCU has another DSPI instance
#if (SPI_INSTANCE_COUNT > 1)
// Set up channels for separate RX/TX DMA requests
#if FSL_FEATURE_DSPI_HAS_SEPARATE_DMA_RX_TX_REQn(1)
        {
            // This channel transfers data from RX FIFO to receiveBuffer
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI1Rx,
                &dspiEdmaState->dmaFifo2Receive ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
            ...
            // Intermediate command/data to TX FIFO (PUSHR).
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI1Tx,
                &dspiEdmaState->dmaCmdData2Fifo ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
        ...
        {
            // This channel transfers data from RX FIFO to receiveBuffer
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI1,
                &dspiEdmaState->dmaFifo2Receive ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
            ...
            // Intermediate command/data to TX FIFO (PUSHR)
            // linked from RX channel. This channel is not activated
            // by dma request, but by channel link from RX.
            if( EDMA_DRV_RequestChannel(
                kEDMAAnyChannel, kDmaRequestMux0Disable,
                &dspiEdmaState->dmaCmdData2Fifo ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
...
// Set up channels for separate RX/TX DMA requests
        ...
        {
            // This channel transfers data from RX FIFO to receiveBuffer
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI2Rx,
                &dspiEdmaState->dmaFifo2Receive ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
            ...
            // Intermediate command/data to TX FIFO (PUSHR).
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI2Tx,
                &dspiEdmaState->dmaCmdData2Fifo ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
        ...
        {
            // This channel transfers data from RX FIFO to receiveBuffer
            if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0SPI2,
                &dspiEdmaState->dmaFifo2Receive ) == kEDMAInvalidChannel )
            {
                return kStatus_DSPI_DMAChannelInvalid;
            }
    ...
    if( EDMA_DRV_RequestChannel( kEDMAAnyChannel, kDmaRequestMux0Disable,
        &dspiEdmaState->dmaSrc2CmdData ) == kEDMAInvalidChannel )
    {
        return kStatus_DSPI_DMAChannelInvalid;
    }

    // Update run-time state struct
    // with the pointer to Software Transfer Control Descriptor
...
dspi_status_t DSPI_DRV_EdmaMasterDeinit( uint32_t instance )
{
    ...
    // This also disables the DSPI module.
    DSPI_HAL_Init( base );
    ...
    NVIC_IntDisable( g_dspiIrqId[ instance ] );

    // Gate the clock for DSPI.
    CLOCK_SYS_DisableSpiClock( instance );

    // Release all of the DMA channels used in the driver.
    // DMA channel structures are stored in the run-time state structure.
    ...
    uint32_t *calculatedDelay )
{
    ...
    *calculatedDelay = DSPI_HAL_CalculateDelay( base, dspiEdmaState->whichCtar,
        whichDelay, dspiEdmaState->dspiSourceClock, delayInNanoSec );
    ...
    const dspi_edma_device_t *device, uint32_t *calculatedBaudRate )
{
    ...
    *calculatedBaudRate = DSPI_HAL_SetBaudRate( base, dspiEdmaState->whichCtar,
    ...
    errorCode = DSPI_HAL_SetDataFormat(
        base, dspiEdmaState->whichCtar, &device->dataBusConfig );
    ...
    uint8_t *receiveBuffer, size_t transferByteCount, uint32_t timeout )
{
    ...
    // Indicates this is a blocking transfer
    dspiEdmaState->isTransferBlocking = true;
    ...
    // If using a shared RX/TX DMA request, then this limits the amount of data
    // we can transfer due to the linked channel.
    ...
    if( DSPI_DRV_EdmaMasterStartTransfer( instance, device, sendBuffer,
        receiveBuffer, transferByteCount ) == kStatus_DSPI_Busy )
    {
        return kStatus_DSPI_Busy;
    }
    ...
    // isTransferInProgress to false and disabling DMA requests and interrupts,
    // then return the timeout error status.
    ...
    uint8_t *receiveBuffer, size_t transferByteCount )
{
    ...
    dspi_edma_master_state_t *dspiEdmaState =
        ( dspi_edma_master_state_t* )g_dspiStatePtr[ instance ];
    // Indicates this is not a blocking transfer
    dspiEdmaState->isTransferBlocking = false;
    ...
    // If using a shared RX/TX DMA request, then this limits the amount of data
    // we can transfer due to the linked channel. The max bytes is 511.
    if( !FSL_FEATURE_DSPI_HAS_SEPARATE_DMA_RX_TX_REQn( instance ) )
    {
        if( dspiEdmaState->bitsPerFrame > 8 )
        {
    ...
    uint32_t *framesTransferred )
{
    ...
    return ( dspiEdmaState->isTransferInProgress ?
             kStatus_DSPI_Busy :
...
void DSPI_DRV_EdmaMasterIRQHandler( uint32_t instance )
{
    ...
    // If the interrupt is due to an end-of-queue, then complete.
    // This interrupt is used during DMA operations
    // and we want to handle this interrupt only when DMA is being used.
    if( DSPI_HAL_GetStatusFlag( base, kDspiEndOfQueue ) )
    {
        // Complete the transfer. This disables the interrupts,
        // so we don't wind up in the ISR again.
        DSPI_DRV_EdmaMasterCompleteTransfer( instance );
    }
}