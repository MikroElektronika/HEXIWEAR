...
int connect_broker (const char *serial_mqtt, const char *password_mqtt)
{
    int len=0;
    int buflen = 0;
    ...
    if (send_wifi_message(buffer, len) == RESP_FALSE)
    {
        return RESP_FALSE;
    }
    return RESP_TRUE;
...
int publish_broker (unsigned char message[], const char *topic)
{
    short packetid = 0;
    int len=0;
    unsigned char buffer[BUFFER_SIZE] = {0};
...
int subscribe_broker (unsigned char topic[])
{
    int len=0;
    unsigned char buffer[BUFFER_SIZE] = {0};
