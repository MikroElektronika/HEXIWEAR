...
{
  statusI2C_t
    status = I2C_Init( &(tslHandle->protocol), tslSettings->address );
                            ...
                            )
{
  ...
  switch ( channel )
  {
    ...
    case CHANNEL_TSL_FULL:  {
                              ...
                              *sensorData = ( dataHigh << 8 ) | dataLow;

                              break;
                            }

    // read ADC channel 1 (IR only)
    case CHANNEL_TSL_INFRARED:  {
                    ...
                    )
{
  ...
  // convert data
  output[idx] = (mE_t)( ( (float)input[idx] ) /* * 100 */ );
}
