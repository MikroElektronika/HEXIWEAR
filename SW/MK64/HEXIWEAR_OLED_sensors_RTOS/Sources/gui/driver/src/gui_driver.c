...
//    status = xQueueSendToBack( GuiDriver_queueHnd, (hostInterface_packet_t*)packet, 100 );
...
//	OSA_EnterCritical( kCriticalDisableInt );
	...
	switch(navigationDir)
	{
      ...
      /**
       * special notification navigation
       */

		  case GUI_NAVIGATION_NOTIFICATION:
		  {
//			  if (
//				  (guiDriver_display.guiItem == &smsScreen)  ||
//			  (guiDriver_display.guiItem == &mailScreen) ||
//			  (guiDriver_display.guiItem == &callScreen)
//				 )
//			  {
//				  return GUI_STATUS_ERROR;
//			  }
//			  if((*(notification_type_t *) param) == NOTIF_TYPE_SMS)
//			  {
//				  ptrNewScreen = &smsScreen;
//			  }
//			  else if((*(notification_type_t *) param) == NOTIF_TYPE_MAIL)
//			  {
//				  ptrNewScreen = &mailScreen;
//			  }
//			  else if((*(notification_type_t *) param) == NOTIF_TYPE_CALL)
//			  {
//				  ptrNewScreen = &callScreen;
//			  }
//
...
//			  transition = OLED_TRANSITION_RIGHT_LEFT;
      ...
      /**
       * special saving power navigation case
       */

      case GUI_NAVIGATION_STANDBY: {
        ptrNewScreen = &power_standbyScreen;
        initParam    = guiDriver_display.guiItem;
        break;
      }
	...
	guiScreen_t*
          oldScreen = guiDriver_display.guiItem;
	guiDriver_display.guiItem = (guiScreen_t*)ptrNewScreen;

	if ( GUI_NAVIGATION_STANDBY != navigationDir )
	{
	...
	while (1)
    {
        statusOLED = OLED_DrawScreen(
                                        ...
                                        width,
                                        height,
                                        ( &power_standbyScreen == oldScreen ) ? OLED_TRANSITION_NONE : transition
                                    );
