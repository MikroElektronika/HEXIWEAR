...
#pragma funcall HostInterface_RxInit HostInterface_RxTask
...
osa_status_t HostInterface_RxInit()
{
    ...
    UART4_bit = 1;
    UART4_C2 |= 1 << 5; // RIE_UART4_C2_bit = 1; //
//    TIE_UART4_C2_bit = 1;
    NVIC_IntEnable( IVT_INT_UART4_RX_TX );
    // Set the interrupt priority level.
    NVIC_SetIntPriority( IVT_INT_UART4_RX_TX, gHostInterfaceIRQPriority_c );
