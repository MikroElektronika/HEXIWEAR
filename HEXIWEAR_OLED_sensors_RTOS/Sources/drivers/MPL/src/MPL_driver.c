 ...
 */
statusMPL_t MPL_Init(
                              handleMPL_t* mplHandle,
                      const settingsMPL_t* mplSettings
                    )
{
  statusI2C_t
    status = I2C_Init( &(mplHandle->protocol), mplSettings->address );
 ...
 */
statusMPL_t MPL_SetOversampleRate (
                                    uint8_t sampleRate
                                  )
{
  if (sampleRate > 7)
 ...
 */
statusMPL_t MPL_EnableEventFlags()
{
  uint8_t
                      ...
                      )
{
  ...
  // convert data
  output[idx] = (float)input[idx] / 16.0;
}
                    ...
                    )
{
  ...
  // convert data
  output[idx] = (mE_t)( ( (float)input[idx] / 16.0 ) * 100 );
}
